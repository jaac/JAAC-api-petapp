...
@Service
public class BoundingCoordinatesService {
	...
	private BoundingCoordinatesService() {
	}
	 ...
	 */
	public BoundingCoordinatesService fromDegrees(double latitude, double longitude) {
		BoundingCoordinatesService result = new BoundingCoordinatesService();
	 ...
	 */
	public  BoundingCoordinatesService fromRadians(double latitude, double longitude) {
		BoundingCoordinatesService result = new BoundingCoordinatesService();
	...
	/**
	 * Computes the great circle distance between this BoundingCoordinatesService instance and the
	 * location argument.
	 * 
	 * @param radius
	 ...
	 */
	public double distanceTo(BoundingCoordinatesService location, double radius) {
		return Math.acos(Math.sin(radLat) * Math.sin(location.radLat)
				+ Math.cos(radLat) * Math.cos(location.radLat) * Math.cos(radLon - location.radLon)) * radius;
	}

	/**
	 * <p>
	 * Computes the bounding coordinates of all points on the surface of a sphere
	 * that have a great circle distance to the point represented by this BoundingCoordinatesService
	 * instance that is less or equal to the distance argument.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param distance
	 *            the distance from the point represented by this BoundingCoordinatesService
	 ...
	 *            approximation of the figure of the Earth is approximately 6371.01
	 *            kilometers.
	 * @return an array of two BoundingCoordinatesService objects such that:
	 *         <ul>
	 ...
	 *         </ul>
	 */
	public BoundingCoordinatesService[] boundingCoordinates(double distance, double radius) {
		...
		return new BoundingCoordinatesService[] { fromRadians(minLat, minLon), fromRadians(maxLat, maxLon) };
	}
